        ...
        var table = $('#table').DataTable({
            ...
            columns: [{
                ...
                render: function(data, type, row, meta) {
                    if (type == 'display') {
                        return `<span data-toggle="tooltip" title="${row.status}" class="badge badge-${row.status == 'success' ? 'success' : 'danger'}">${'CAP-' + data}</span>`
                    } else {
                        return data
                    }
                }
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    if (result.status == true) {
                        ...
                        $('.title-edit').remove()
                        $('#titleEdit').append(
                            `<span class="badge title-edit ml-2 badge-${result.data.status == 'success' ? 'success' : 'danger'}">CAP-${result.data.id}`
                        )
                        if (show) {
